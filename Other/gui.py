...
class App(customtkinter.CTk):
    ...
    def run_models(self, data_dir, model_choice, order_level_model, ungulates_model, small_carnivores_model):
        s2 = time.time()
        num_images = len(os.listdir(data_dir))
        # json_dir = r"D:\WII\Experiment\output.json"
        json_dir = self.megadetector(data_dir)
        ...
        if data_type == 1:
            ...
            for video in videos:
                ...
                if not model_choice == 0:
                    self.run_models(frames_dir, model_choice, order_level_model, ungulates_model, small_carnivores_model)
                else:
                    self.run_models(frames_dir, model_choice)

        if data_type == 0:
            if not model_choice == 0:
                self.run_models(data_dir, model_choice, order_level_model, ungulates_model, small_carnivores_model)
            else:
                self.run_models(frames_dir, model_choice)
        return
        ...
        with Popen(command,
            ...
            for line in p.stdout:
                if line.startswith("Loaded model in"):
                    self.output_label.configure(text = "")
                    self.output_label.configure(text = f"Predicting objects in {len(os.listdir(img_dir))} images...\n")
                    self.update()
    ...
    def shift_original_images_model1(self, img_path, df):
        for i in range(len(df)):
            ...
            cat_dir = os.path.join(img_path, species)
    ...
    def create_output_files_model2(self, img_path, df, order_level_model):
        ...
        for i in range(len(df)):
            ...
            if conf > 0.1:
                ...
                pred_class = "Others"
                order_level_pred_prob = 0
                pred_probs.append(0)
            
            order_level_categories.append(pred_class)
            animal_dir = os.path.join(cropped_dir, pred_class)
            if not os.path.exists(animal_dir):
                os.makedirs(animal_dir)

            cropped_img_path = os.path.join(animal_dir, f"{d_num}_{conf}_{order_level_pred_prob}_{filename}")
    ...
    def shift_original_images_model2(self, img_dir, df):
        ...
        for i in range(len(df)):
            ...
            animal_folder = os.path.join(img_dir, order_level_class)
